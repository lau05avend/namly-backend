...
import { isAnonymousAuthUser } from './is-anonymous-auth-user.util';
...
export function extractAuthJwtClaims(user: User): AuthJwtClaims {
  ...
  const provider = isAnonymous ? 'anonymous' : readString(user.app_metadata?.provider);
  const externalId = isAnonymous ? user.id : readString(user.user_metadata?.provider_id);
